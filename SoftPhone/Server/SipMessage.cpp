...
int CSipMessage::GetAnwStr(char *registStr,unsigned int *len,SipMode mode)
{
	...
	case CMD400:
		...
		break;
	case CMD302:
		m_pMessage->status_code = 302;
		osip_message_set_reason_phrase(m_pMessage,strdup("Moved Temporarily"));
		break;
	case CMD404:
		m_pMessage->status_code = 404;
		osip_message_set_reason_phrase(m_pMessage,strdup("NOT FOUNT"));
	default:
	    break;
	}
	char *str = NULL;
